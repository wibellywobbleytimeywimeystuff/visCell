...
Modul: counting.py
Beschreibung:
Dieses Skript verarbeitet Mikroskopiebilder und extrahiert Merkmale zur Zellzählung.
...
def count_from_maps(
    ...
    num_labels, seed_markers = cv2.connectedComponents((seed_peaks > 0).astype(np.uint8))
    if num_labels <= 1:
        return {c: 0 for c in CLASSES}
    ...
    for region_id in range(1, markers.max() + 1):
        ...
        for ci, cls in enumerate(CLASSES):
            class_scores.append(float(centers_crop[..., ci][region_mask].mean() if area > 0 else 0.0))
