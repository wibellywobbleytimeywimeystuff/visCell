"""Modul: peaks.py
Beschreibung:
Dieses Skript enthält Hilfsfunktionen zur Spitzen-Erkennung in binären Karten, um Zellzentren aus Vorhersagekarten zu extrahieren.
...
def find_peaks(binary_map_u8: np.ndarray, min_dist: int = 12) -> np.ndarray:
    ...
    binary_clean = cv2.morphologyEx(
        binary_map_u8, cv2.MORPH_OPEN, kernel_3, iterations=1
    )

    dist_map = cv2.distanceTransform(
        (binary_clean > 0).astype(np.uint8), cv2.DIST_L2, 5
    )
