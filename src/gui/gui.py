...
colormode = ("Hellmodus")  # Button-Text
ctk.set_appearance_mode(appearance)  # globales Theme setzen
ctk.set_default_color_theme("blue")  # Farb-Theme: blau
...
class App(ctk.CTk):
    ...
    # =========================
    # 5 ) Helper: Background für Live/Placeholder setzen
    # =========================
    def _set_live_background(self, color: str):
        ...
        # Tk Widgets Hintergrund setzen (weil ctk.CTkFrame / ctk.CTkLabel separate bg-Property haben)
        ...
        # =========================
        # 6 ) Farbschema Toggle (Hell/Dunkel)
        # =========================
        # ===== Toggle: Validation =====
    ...
    # =========================
    # 7 ) Footer (unten rechts: Theme Toggle Button)
    # =========================
    ...
    def _start_camera(self, index: int):
        ...
        self.cap = cv2.VideoCapture(index, cv2.CAP_DSHOW)   # Erst alles stoppen (inkl. after cancel), damit nichts parallel läuft
    ...
    # =========================
    # 9 ) Live Loop (liest Frames und zeigt sie)
    # =========================
    ...
    # =========================
    # 10 ) Snapshot (speichert in captures/)
    ...
    # =========================
    # 11 ) Bild importieren (Dialog startet in captures/)
    # =========================
    def _import_image(self):
        self._stop_camera() # Kamera anhalten, damit Live nicht direkt das importierte Bild überschreibt
    ...
    # =========================
    # 12 ) Bild exportieren (Save-Dialog; startet in captures/)
    # =========================
    def _export_image(self):
        ...
        # ===== Speichern-Dialog =====
        save_path = filedialog.asksaveasfilename(title="Bild exportieren",minitialdir=self.capture_dir, initialfile=default_name, defaultextension=".png",
            filetypes=[
                ("PNG", "*.png"),
                ("JPEG", "*.jpg *.jpeg"),
                ("BMP", "*.bmp"),
    ...
    # =========================
    # 13 ) Optional: Alles aus captures/ exportieren (kopieren)
    # =========================
    # ===== Ordner-Dialog, um Zielordner auszuwählen =====
    def _export_all_captures(self):
        target_dir = filedialog.askdirectory(title="Zielordner wählen (alle Captures exportieren)", initialdir=os.path.abspath(self.capture_dir))
    ...
    def _open_metadata_popup(self, analysis_label: ctk.CTkLabel):
        ...
        # Titel im Popup
        title_label = ctk.CTkLabel(popup, text="Metadaten bearbeiten", font=ctk.CTkFont(size=18, weight="bold"))
    ...
    def _build_gui(self):
        ...
        # Freeze Checkbox
        self.freeze_checkbox = ctk.CTkCheckBox(controls_frame, text="Nach Aufnahme pausieren", variable=self.freeze_after_capture_var, command=self._on_freeze_toggle)
        ...
        # ctk.CTkFrame Container (damit ctk.CTkLabel sauber das Bild anzeigen kann)
        self.video_container = ctk.CTkFrame(live_view, bg=self.placeholder_bg, bd=0, highlightthickness=0)
        self.video_container.grid(row=0, column=0, sticky="nsew")
        self.video_container.grid_rowconfigure(0, weight=1)
        self.video_container.grid_columnconfigure(0, weight=1)

        # ctk.CTkLabel: hier wird das Bild (PhotoImage) angezeigt
        self.video_label = ctk.CTkLabel(self.video_container, bg=self.placeholder_bg, bd=0, highlightthickness=0, relief="flat",)
        self.video_label.grid(row=0, column=0, sticky="nsew")

        # Overlay Text über dem Live View
        self.video_text = ctk.CTkLabel(live_view, text="Kein Bild vorhanden\n(bitte Kamera auswählen)", font=ctk.CTkFont(size=18), fg_color="transparent",)
        self.video_text.place(relx=0.5, rely=0.5, anchor="center")
        ...
        right_panel.grid_propagate(False)  # verhindert, dass der Frame sich automatisch anpasst
        ...
        analysis_label = ctk.CTkLabel(analysis_frame,
            text=(
                "ANALYSE_ERGEBNIS\n\n"
                "Datum:  \nPrüfer:  \nLabor: \n\n"
                "Mikroskop:   \nVergrößerung:   \nProbe Nummer:   \n\n"
        ...
        ctk.CTkButton(export_frame, text="Metadaten ändern", command=lambda: self._open_metadata_popup(analysis_label),).pack(side="left", pady=10)
        ...
        self.progress_frame, text="0 %", font=ctk.CTkFont(size=16, weight="bold"),)
    ...
    # =========================
    def _on_close(self):
        self._stop_camera()  # Kamera stoppen und Ressourcen freigeben
