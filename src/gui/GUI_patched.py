...
class App(ctk.CTk):
    ...
    def _render_analysis_text(self) -> str:
        ...
        return (
            ...
            f"Erythrozyten Anzahl: {int(getattr(self, 'ki_ery', 0))}\n"
            f"Leukozyten Anzahl: {int(getattr(self, 'ki_leuko', 0))}\n"
            f"Hefezellen Anzahl: {int(getattr(self, 'ki_hefe', 0))}\n\n"
    ...
    # Hilfe-PDF öffnen
    # =========================
    def _open_benutzerhandbuch(self):
        pdf_path = self.manual_path

        if not pdf_path.exists():
            messagebox.showerror(
                "Benutzerhandbuch",
                f"Die PDF wurde nicht gefunden:\n{pdf_path}"
            )
            return
        ...
        try:
            ...
            self._set_status("Benutzerhandbuch geöffnet")
    ...
    def _apply_all_adjustments(self, bgr: np.ndarray) -> np.ndarray:
        ...
        if (
            abs(float(self.contrast_alpha) - 1.0) < 1e-9 and
    ...
    def start_analysis(self):
        if self.last_frame is None:
            self._set_status("Kein Bild vorhanden – erst Kamera wählen / Bild importieren")
            return
        ...
        try:
            self._show_progress()
            self._status("KI analysiert Bild...", val_loadcolor)
    ...
    def _analysis_process(self):
        try:
            ...
            self.after(0, lambda: self.analysis_label.configure(text=self._render_analysis_text()))
            self.after(0, lambda: self._set_status("Analyse abgeschlossen ✅"))
            try:
                self._update_progress(3, 3)
                self._status("Analyse abgeschlossen", val_refcolor)
            except Exception:
                pass
    ...
    def _validation_process(self):
        ...
        ok_ery, tol_ery = self._regel_check(ist=ist, soll=soll)
        self.validierung_ok = bool(ok_ery)
        tol = tol_ery
    ...
    def _build_gui(self):
        ...
        stain_values = ["Kein Färbemittel", "Giemsa", "Gram", "May-Grünwald", "Methylenblau"]
        self.stain_select = ctk.CTkOptionMenu(
            controls_frame,
            values=stain_values,
            variable=self.stain_var
        )
        self.stain_select.grid(row=0, column=3, padx=5)
        ...
        self.cam_status_label.grid(row=0, column=4, padx=10, sticky="w")

        self.help_button = ctk.CTkButton(
            controls_frame,
            text="Hilfe",
            command=self._open_benutzerhandbuch,
            width=60,
        ...
        self.help_button.grid(row=0, column=5, padx=(10, 5), sticky="e")
